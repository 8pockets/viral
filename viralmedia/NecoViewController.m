...
#import "NecoViewController.h"

@interface NecoViewController (){
...
@implementation NecoViewController
...
- (void)viewDidLoad
{
    ...
    self.nekoTable.delegate = self;
    self.nekoTable.dataSource = self;
    
    //ヘッダー画像
    self.navigationItem.titleView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"navtitle.png"]];
    
    //カスタムセルの設定
    UINib *nib = [UINib nibWithNibName:@"NecoCell" bundle:nil];
    [self.nekoTable registerNib:nib forCellReuseIdentifier:@"NecoCell"];
    [self.searchDisplayController.searchResultsTableView registerNib:nib forCellReuseIdentifier:@"NecoCell"];
    ...
    if ([Weeksave arrayForKey:@"Necosave"]) {
        NSLog(@"%@",@"DATA!!!");
        for (NSData *object in [Weeksave arrayForKey:@"Necosave"]) {
    ...
    [self.nekoTable addSubview:_refreshControl];
...
{
    NecoCell *cell = [tableView dequeueReusableCellWithIdentifier:@"NecoCell"];
...
- (void)refresh {
    ...
    [manager GET:@"http://necobuzz.com/cat.json"
      parameters:nil
         success:^(AFHTTPRequestOperation *operation, id responseObject) {
             ...
             [Newsave setObject:archivearray forKey:@"Necosave"];
             [Newsave synchronize];
